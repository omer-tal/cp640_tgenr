...
from keras.layers import Conv1D, MaxPooling1D, Flatten,Input, Reshape, Embedding,merge, Dropout
...
INPUT_SIZE = 141
PATH = "preprocessed/combined_model/"+ str(INPUT_SIZE) + "k/"
...
def generate_instances(batch_size,u_input,p_input,r_input,u_reviews,p_reviews,u_graph_emb,p_graph_emb,total_users,total_pois,
                       u_emb_size,p_emb_size,emb_size, training,pace,deepconn):
    """
    ...
    while 1:
        ...
        for j in range(batch_size):
            ...
            # If the model is based on PACE or NeuMF, it uses user and poi as input
            if (pace>=1):
                features['user_input'].append(u_input[i])
                features['poi_input'].append(p_input[i])
            # If the model is based on DeepCONN, it uses written reviews by user and poi as input
            if (deepconn):
            ...
            if (training):
                ...
                # If the model is based on PACE algorithm, the output includes user and POI graphs
                if (pace==2):
...
# For MF based NeuMF, set pace=1, mf=1 and deepconn=False
# For MLP based NeuMF, set pace=1, mf=2 and deepconn=False
# For PACE with no contextual regularization set pace=1, mf=0 and deepconn=False
# For PACE with contextual regularization set pace=2, mf=0 and deepconn=False
# For deepconn, set pace=0, deepconn=True and mf=0
# For the combined model by me, set pace=2, deepconn=True and mf=0
pace = 2
deepconn = True
mf = 0

# Define the model name used in saving it to disk
model_str = str(CONTEXT_LAYERS[0])
if (pace>0 and mf>0):
    model_str += "_mf" + str(mf)
elif (pace>0):
    model_str += "_pace" + str(pace)
if (deepconn):
    model_str += "_conn" + str(REVIEWS_HIDDEN_LAYER_SIZE)
...
# Defining structure for pace/neumf based model
if pace>=1:
    ...
    # If pace==2, add contextual regularization as output
    if (pace==2):      
        ...
        pace_outputs = pace_outputs + [user_context_output,poi_context_output]
    ...
    # If this is a MF based neumf
    if (mf==1):
        # Perform dot product of the two embedded layers
        merged_input = merge([user_latent_flattened, poi_latent_flattened],mode='mul',name='merged_input')
        hidden_layer = merged_input
    # If this model has hidden layers on top of concatentation (neumf with MLP or pace)
    else:
        # Merging the user and poi for the input and reviews
        merged_input = merge([user_latent_flattened, poi_latent_flattened],mode='concat',name='merged_input')
        hidden_layer = merged_input
...
# Setting parameters if DeepCONN model is used
if deepconn:
    ...
    merged_reviews = merge([user_reviews_dense,poi_reviews_dense], mode='concat', name='merged_reviews')

# If the output is based on combining the model
if pace>=1 and deepconn:
...
# If the model is based on neumf or pace, the output is a sigmoid transformation of the last hidden layer
elif pace>=1:
    review_output = Dense(1, activation='sigmoid', kernel_initializer='lecun_uniform', name = 'review_output')(hidden_layer)
# If the model is DeepCONN, combine dot product and concatenation (no factorization machines layer in keras)
elif deepconn:
...
model_outputs = [review_output] + pace_outputs
...
early_stop = EarlyStopping(monitor=monitor, min_delta=0.001, patience=10, verbose=1, mode='max')
# Save the model every time the validation accuracy improves
checkpoint = ModelCheckpoint(CHECKPOINT_PATH,monitor=monitor,save_best_only=True, verbose=1, 
...
history = model.fit_generator(generate_instances(BATCH_SIZE,train['user'],train['poi'],train['review'],embedded_user_reviews,
                                       embedded_poi_reviews,user_graph_emb,poi_graph_emb,total_users,total_pois,
                                                 user_embedded_size,poi_embedded_size,WORD_EMBEDDING,True,pace,deepconn),
                              validation_data = generate_instances(BATCH_SIZE,validation['user'],validation['poi'],
                                                                   ...
                                                                   poi_embedded_size,WORD_EMBEDDING,True,pace,deepconn),

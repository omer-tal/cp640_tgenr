...
locations = load_poi_file(POI_FILE,POIS_LIMIT)
...
new_locations,new_data,dictionary["poi_to_pid"] = poi_to_pid(locations,data,poi_reviews_count,MIN_POI_REVIEWS,filtered_graph)
...
filtered_graph2,dictionary['user_to_uid'] = user_to_uid(filtered_graph2)
...
USER_PER = 5
POI_PER = 5
# Number of words will be determined by the percentiles
user_words_perc = int(np.percentile(get_counts(reviews['user_reviews']), USER_PER))
poi_words_perc = int(np.percentile(get_counts(reviews['poi_reviews']), POI_PER))
print("Chosen number of words to represent a user and poi are:",user_words_perc, poi_words_perc)
...
user_sampled = random_walk(filtered_graph2,0.1,15,20,3)
graph_stats(user_sampled)
poi_sampled = random_walk(poi_graph,0.1,15,30,3)
